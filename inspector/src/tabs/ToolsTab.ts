...
import GIFExporter from '../../../dist/GIFExporter/js/gif.exporter.js'; // tslint:disable-line
...
export class ToolsTab extends Tab {
	...
	constructor(tabbar: TabBar, insp: Inspector) {
		...
		title.textContent = 'Capture';
		{
			...
			const recorder = new GIFExporter(insp.scene.getEngine(), { delay: 50, duration: 2000 });
			let gifRecorder = Inspector.DOCUMENT.createElement('input');
			gifRecorder.value = 'Create Animated GIF';
			gifRecorder.type = 'button';
			gifRecorder.className = 'tool-input';
			gifRecorder.onclick = () => {
				if (this._scene.activeCamera) {
					recorder.download('babylon.gif');
				}
			};
			elemValue.appendChild(gifRecorder);
		}
	}
